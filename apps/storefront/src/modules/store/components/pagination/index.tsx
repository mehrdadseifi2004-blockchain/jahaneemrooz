...
export function Pagination({
      ...
      <button
        ...
        className={`flex h-10 min-w-10 items-center justify-center rounded-xl border px-3 text-sm font-bold transition ${
          isCurrent
            ...
            : "border-[var(--theme-border)] bg-[var(--theme-surface)] text-[var(--theme-text-muted)] hover:border-[#ff5a00]/50 hover:text-[#ff7a1a]"
        }`}
      >
        {pageNumber}
      </button>
  ...
  const renderEllipsis = (key: string) => (
    <span
      key={key}
      className="flex h-10 min-w-8 items-center justify-center text-[var(--theme-text-subtle)]"
      aria-hidden="true"
    >
      …
    </span>
  ...
  const navigationButtonClass =
    "inline-flex h-10 items-center justify-center rounded-xl border border-[var(--theme-border)] bg-[var(--theme-surface)] px-4 text-sm font-bold text-[var(--theme-text-muted)] transition hover:border-[#ff5a00]/50 hover:bg-[#ff5a00] hover:text-white disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-[var(--theme-border)] disabled:hover:bg-[var(--theme-surface)] disabled:hover:text-[var(--theme-text-muted)]"

  return (
    <nav
      aria-label={dictionary.store.pagination.ariaLabel}
      className="mt-10 flex w-full items-center justify-between border-t border-[var(--theme-border)] pt-5"
      data-testid={dataTestid}
    >
      <button
        type="button"
      ...
      <span className="text-sm text-[var(--theme-text-muted)] small:hidden">
        {mobilePageLabel}
      </span>
